...
// Error Handlers //error handler, error를 check하는게 아닌 발새안 것을 알고 이를 부르면 처리해주는 함수
static void RedefinitionError(char *name, int lineno, SymbolList symbol) // 같은 name이더라도 scope가 다르면 정의할 수 있음 => 이것도 고려해봐야 할 듯
{
	Error = TRUE; // main 에 정의되어 있는 전역변수
	fprintf(listing, "Error: Symbol \"%s\" is redefined at line %d (already defined at line ", name, lineno);
	int First = TRUE; // int가 ture일 수 있나?
	// symbol table은 linked list로 정의되어 있음
	// 못찾는 경우도 생길 수 있음, 이 함수를 호출하려면 반드시 error가 발생한 것을 확인하고 call해야함
	while (symbol != NULL) 
	{
 ...
 */
// void (*preProc)(TreeNode *) : 함수 포인터
static void traverse(TreeNode *t, void (*preProc)(TreeNode *), void (*postProc)(TreeNode *))
{
	if (t != NULL)
	{
		// pre-order process // pre-order : 노드 -> left-child -> right-child
		preProc(t);
		// traverse childs
		{
			int i;
		...
		// 이게 왜 post-order ??? 그냥 마지막에 실행하는거 아님?
		postProc(t); // scopeOut(syntaxTree)
...
static void insertNode(TreeNode *t) 
{
	switch (t->kind)
	{
		...
		case VariableDecl:
		{
			...
			// 현재 scope의 symbol table에 symbol을 넣어줌
			// Break
			break;
		}
...
// input function, output function은 뭐지?
void declareBuiltInFunction(void) // 왜 있는 거? 
{
	...
	inputFuncNode->child[0]->flag = TRUE; // params type이 void인 경우 flag : TRUE
...
// symbol table은 hash table + tree structure
// scope는 tree로 구현되어 있고 tree의 각 node(scope)에서는 hash table로 구현되어 있음
// hash table은 collosion의 해결 방법으로 linked list를 사용
void buildSymtab(TreeNode *syntaxTree) 
{
	// Initialize Global Variables 
	// static ScopeRec *globalScope = NULL;
	// static ScopeRec *currentScope = NULL;
	globalScope = insertScope("global", NULL, NULL); // 제일 위에 있는 root node, 제일 위의 scope가 global
	currentScope = globalScope;

	declareBuiltInFunction(); // 왜 하는 거? //
	// input function node 
	//		user로 부터 받은 integer 값을 리턴
	// output function node 설정해둠
	//		받은 argument의 값을 print

	// insert node all
	traverse(syntaxTree, insertNode, scopeOut);
	// syntaxTree는 이전의 parsing에서 만들어준 것을 사용하는 것
	// insertNode를 실행 paramenter로 syntaxTree를 넣어줌
	// syntaxTree를 pre-order로 insertNode 실행
	// scopeOut을 제일 나중에 실행
	// insertNode :
	// scopeOut : 현재 scope에서 나가고 부모 scope로 바꿔줌

	// trace
	if (TraceAnalyze) // false임
	{
		fprintf(listing, "\n\n");
