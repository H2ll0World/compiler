...
//--------------------------------------------------------
// Insert New Scope
// Scope는 tree로 구현되어 있음
ScopeRec *insertScope(char *name, ScopeRec *parent, TreeNode *func) // func는 현재 scope에 잇는 function?
{
	// Error Check: Parameters
	ERROR_CHECK( name != NULL || parent != NULL ); // null이 없거나 parent가 없으면 error를 print
...
{
	// Error Check: Parameters
	// marco로 정의되어 있고, 딱히 하는 건 없고, error를 print해줌
	ERROR_CHECK( currentScope != NULL && name != NULL );

	// Find Symbol Table Entry: Just Find in Current Scope Only
	int hashIdx = hash(name); 
	SymbolRec *lastSymbol = currentScope->symbolList[hashIdx]; // 이게 collison이 발생해서 linked list형식으로 뒤에 달려있을 수도 있는거 아님?
	SemanticErrorState state = STATE_NORMAL;
	
	// lastSymbol == NULL인 경우 : 해당 bucket이 비어있는 경우
	// 해당 bucket에 값이 존재하는 경우 
	// 이미 존재하는 name : redefinition error
	// collison발생 : linked list로 이를 해결해주면 됨
	// state를 언제 지정해 주는 지 알아보면 쉬울 듯
	while (lastSymbol != NULL) 
	{
		// If Duplicated Symbol Exist
		// redefiniton error인 경우
		if( strcmp(name, lastSymbol->name) == 0 ) // strcmp는 두 string이 동일한 경우 0을 return
		{
			// 이미 STATE_REDEFINED일 수가 있나? => 에러가 발생해도 이를 처리해주는 것이 아니락, error가 발생했음을 표시해 두는 것?
			...
			else ERROR_CHECK( "Normal, Duplicated Symbol Exist"); // lastSymbol->state == STATE_NORMAL인 경우
		}
		// Iterate Symbol List
